...
pub struct Opl4 {
    ...
    use_count: [usize; 4], // [two-ops, four-ops, rhythm, PCM]
    dual: bool,
    drum: [u8; 2],
    sam: [u16; 2],
    tone: u16,
...
impl Opl4 {
    ...
    fn poke(&self, id: usize, addr: u8, data: u8, writer: &mut VgmWriter) {
        ...
        // OPL4 command format: 0xD0, port/instance byte, address, data
        let port_byte = ((id & 1) | ((id & 2) << 6) | ((id & 8) >> 2)) as u8;
        let _ = writer.write_data(&[0xD0, port_byte, addr, data]);
    }
...
impl SoundChip for Opl4 {
    ...
    fn file_begin(&mut self, writer: &mut VgmWriter) {
        ...
        self.dual = self.use_count[2] > 1 || self.use_count[3] > 24;
        ...
        self.poke(1, 0x05, 0x03, writer); // OPL3 mode + OPL4 mode enable
        self.poke(2, 0x01, 0x20, writer); // Second chip waveform select
        self.poke(3, 0x05, 0x03, writer); // Second chip OPL3+OPL4 mode
        ...
        // Initialize PCM part
        self.poke(8, 0x00, 0x00, writer);
        self.poke(10, 0x00, 0x00, writer);
        self.poke(8, 0x01, 0x00, writer);
        self.poke(10, 0x01, 0x00, writer);
        self.poke(8, 0x02, 0x30, writer);
        self.poke(10, 0x02, 0x20, writer);
    ...
    fn file_end(&mut self, writer: &mut VgmWriter) {
        ...
        header.write_u32(offset::YMF278B_CLOCK, clock_val);
    }
